...
public class ClassFirebaseController {
    ...
    public Task<QuerySnapshot> getClassFromId(int _id){
        Query documentQuery = this.firebaseFirestore.collection(Class.COLLECTION_CLASS).whereEqualTo("id_", _id);
        return documentQuery.get();
    }

    public Task<QuerySnapshot> getClassesFromInstitutionId(int _id){
        Query documentsQuery = this.firebaseFirestore.collection(Class.COLLECTION_CLASS).whereEqualTo("institutionId", _id);
